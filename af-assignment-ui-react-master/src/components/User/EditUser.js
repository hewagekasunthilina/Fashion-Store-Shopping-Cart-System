...
class EditUser extends React.Component {
	...
	onDeleteBtnClick() {
		let userToDelete = { name: this.state.name, email: this.state.email, password: this.state.password, role: this.state.role };
	...
	onTextChange(e) {
		...
		switch (field) {
			case "email": this.setState({ email: newValue }); break;
		...
		return (
			...
			>
              ...
              <div class="form-group" style={{ "margin-right": "1rem" }}>
								...
								<select ref={this.role} value={this.state.role} disabled={this.state.role === "customer"} onChange={this.onSelectChange} className="form-control">{ this.state.roles.map((role, index) => { return(<option>{role}</option>) })} </select>
							</div>
							<div class="form-group" style={{ "margin-right": "1rem" }}>
								<button
									href="#"
