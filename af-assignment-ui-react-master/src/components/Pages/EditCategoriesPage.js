...
class EditCategoriesPage extends React.Component {
  ...
  updateCategory(category) {
    ...
    .catch(error => { alert(`Unable to update ${category.name} because ${error}`) });
  }
  ...
  deleteCategory(category) {
    ...
    .catch(error => { alert(`Unable to delete ${category.name} because ${error}`) });
  }

  fetchItems() {
    axios.get('/categories')
    .then(res => { console.log(res.data.body); this.setState({ categories: res.data.body }); })
  }
    ...
    return (
      <div className="row align-items-center">
        {this.state.categories.map((category, index) => {
          return <EditCategory category={category} updateCategory={this.updateCategory} deleteCategory={this.deleteCategory} />;
        })}
      </div>
    );
  }
