...
class EditItem extends React.Component {
  ...
  onFieldChange(e) {
    ...
    switch(updatedField) {
      case "name": this.setState({ title: newValue.value }); break;
      case "price": this.setState({ price: newValue.value }); break;
      case "description": this.setState({ body: newValue.value }); break;
      case "discount": this.setState({ discount: newValue.value }); break;
      case "price": this.setState({ price: newValue.value }); break;
      case "delete": this.setState({ deleteConfirmation: newValue.checked }); break;
      case "category": this.setState({ category: newValue.value }); break;
    }
  }
  ...
  validateForm() {
    ...
    Object.keys(itemToBeSubmitted).forEach(key => { if (itemToBeSubmitted[key] === "") hasEmptyField = true });
            ...
            <div class="form-inline">
              ...
              <div class="form-group" style={{ "margin-right": "1rem" }}>
                ...
                <select 
                  ...
                  ref={this.category} 
                  className="form-control" >
                    { this.state.categories.map((category, index) => { return(<option>{category.name}</option>) })} 
                  </select>
              </div>
              <div class="form-group" style={{ "margin-right": "1rem" }}>
